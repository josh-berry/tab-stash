import {ExtensionTypes, Runtime, Tabs as T, Windows as W} from 'webextension-polyfill';
...
class State {
    ...
    readonly onTabUpdated: events.MockEvent<(tabId: number, changeInfo: T.OnUpdatedChangeInfoType, tab: Tab) => void>;
    readonly onTabMoved: events.MockEvent<(tabId: number, moveInfo: T.OnMovedMoveInfoType) => void>;
    readonly onTabActivated: events.MockEvent<(activeInfo: T.OnActivatedActiveInfoType) => void>;
    readonly onTabHighlighted: events.MockEvent<(highlightInfo: T.OnHighlightedHighlightInfoType) => void>;
    readonly onTabDetached: events.MockEvent<(tabId: number, detachInfo: T.OnDetachedDetachInfoType) => void>;
    readonly onTabAttached: events.MockEvent<(tabId: number, attachInfo: T.OnAttachedAttachInfoType) => void>;
    readonly onTabRemoved: events.MockEvent<(tabId: number, removeInfo: T.OnRemovedRemoveInfoType) => void>;
    readonly onTabReplaced: events.MockEvent<(addedTabId: number, removedTabId: number) => void>;
    readonly onTabZoomChange: events.MockEvent<(ZoomChangeInfo: T.OnZoomChangeZoomChangeInfoType) => void>;
    ...
    constructor() {
        ...
        this.onWindowFocusChanged = new events.MockEvent('browser.windows.onFocusChanged');
    ...
    validate() {
        ...
        for (const w of this.windows) {
            ...
            for (const t of w.tabs) {
                // istanbul ignore if
                if (t.active) {
                    if (active_tab) throw new Error(`Multiple active tabs in window ${w.id}`);
                    active_tab = t;
                }
                // istanbul ignore if
                if (t.windowId !== w.id) {
                    throw new Error(`Inconsistent windowId for tab ${t.id} in window ${w.id}`);
                }
                // istanbul ignore if
...
class MockTabs implements T.Static {
    ...
    readonly onUpdated: events.MockEvent<(tabId: number, changeInfo: T.OnUpdatedChangeInfoType, tab: Tab) => void>;
    readonly onMoved: events.MockEvent<(tabId: number, moveInfo: T.OnMovedMoveInfoType) => void>;
    readonly onActivated: events.MockEvent<(activeInfo: T.OnActivatedActiveInfoType) => void>;
    readonly onHighlighted: events.MockEvent<(highlightInfo: T.OnHighlightedHighlightInfoType) => void>;
    readonly onDetached: events.MockEvent<(tabId: number, detachInfo: T.OnDetachedDetachInfoType) => void>;
    readonly onAttached: events.MockEvent<(tabId: number, attachInfo: T.OnAttachedAttachInfoType) => void>;
    readonly onRemoved: events.MockEvent<(tabId: number, removeInfo: T.OnRemovedRemoveInfoType) => void>;
    readonly onReplaced: events.MockEvent<(addedTabId: number, removedTabId: number) => void>;
    readonly onZoomChange: events.MockEvent<(ZoomChangeInfo: T.OnZoomChangeZoomChangeInfoType) => void>;
    ...
    async update(tabId: number | undefined, updateProperties: T.UpdateUpdatePropertiesType): Promise<T.Tab>;
    ...
    async update(tabId: any, updateProperties?: any): Promise<T.Tab> {
        ...
        if (typeof tabId !== 'number') {
            throw new Error(`Calling tabs.update() without a tab ID is not implemented`);
        }
        const options: T.UpdateUpdatePropertiesType = updateProperties;
        // istanbul ignore if
        if (! options) {
            throw new Error(`Calling tabs.update() without options is not implemented`);
        }
    ...
    async move(tabIds: number | number[], moveProperties: T.MoveMovePropertiesType): Promise<T.Tab | T.Tab[]> {
        ...
        for (const tid of tab_ids) {
            ...
            if (from_win.tabs[tab.index] !== tab) {
                throw new Error(`BUG: Window ${from_win.id} does not have tab ${tab.id} at index ${tab.index}`);
            }
            ...
            if (from_win !== to_win) {
                this.onAttached.send(tid, {
                    newWindowId: to_win.id, newPosition: to_index});
            } else {
                this.onMoved.send(tid, {
                    windowId: to_win.id, fromIndex: tab.index, toIndex: to_index});
            }
            ++to_index;
        }
