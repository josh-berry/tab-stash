...
const Main = Vue.extend({
    ...
    computed: {
        recently_updated(): undefined | 'features' | 'fixes' {
            if (this.local_options.last_notified_version === this.my_version) {
                return undefined;
            }
            const my = parseVersion(this.my_version);
            const last = this.local_options.last_notified_version
                ? parseVersion(this.local_options.last_notified_version)
                : [];
