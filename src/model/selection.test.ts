...
class TestModel implements SelectableModel<string> {
    ...
    isSelected(s: string): boolean { return this.selection.value.has(s); }
    async clearSelection(): Promise<void> {
        this.selection.value = new Set();
    }

    async setSelected(items: Iterable<string>, isSelected: boolean): Promise<void> {
        if (isSelected) {
...
describe('model/selection', () => {
    ...
    async function expectSelectedItems(topSel: string[], bottomSel: string[]) {
        ...
        expect(model.selectedCount.value).to.equal(topSel.length + bottomSel.length);
    }
